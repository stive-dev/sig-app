...
<html lang="en">
  ...
  <body>
    ...
    <div style="padding: 20px; color: #ffffff;">
      <div style="display: flex; gap: 20px; width: 100%; padding-bottom: 0px;">
        <div style="border-radius: 10px; padding: 10px; box-sizing: border-box; flex-basis: 30%; flex-grow: 1; background-color: #313348; display: flex; align-items: center; gap: 20px; justify-content: space-between;">
          <div><h2>MODIFICAR MATERIA PRIMA EN EL SISTEMA</h2></div>
          <div>
            <div class="">
              <a href="<?php echo __URL__ . '/materias_primas'; ?>" class="navbar__option navbar__option--active">Volver</a>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="form__container">
      <form class="form__content" action="<?php echo __URL__ . '/materias_primas/update'; ?>" method="POST">
        <div class="form__controls">
          <label class="form__label" for="">Id</label>
          <input class="form__control" name="id" type="text" value="<?= $materiaPrima->id ?>" placeholder="00000" style="pointer-events:none;" />
        </div>
        <div class="form__controls">
          <label class="form__label" for="">Tipo</label>
          <input class="form__control" name="tipo" type="text" value="<?= $materiaPrima->tipo ?>"/>
        </div>
        <div class="form__controls">
          <label class="form__label" for="">Proveedor</label>
          <input class="form__control" name="proveedor" type="text" value="<?= $materiaPrima->proveedor ?>"/>
        </div>
        <div class="form__controls">
          <label class="form__label" for="">Descripcion</label>
          <input class="form__control" name="descripcion" type="text" value="<?= $materiaPrima->descripcion ?>"/>
        </div>
